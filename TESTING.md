...
Result keys	|	To be shown accordingly to the computed result, allowing the player to understand their guess result.	|	I set the secret code visible and observed the results computed in comparison with the color sequence that I was choosing.	|	They were correctly shown, according to the computed result.	|	 Pass 
